...
class GeometryUtils:
    ...
    @staticmethod
    def calculate_rectangles_top_edge_angle(rot_rect1, rot_rect2):
        """
        Calculate the angle between the top edges of two rectangles.
        The top edge is defined as the edge with smaller y values.
        
        Args:
            rot_rect1: First rotated rectangle ((cx, cy), (w, h), angle_deg) or [cx, cy, w, h, angle_rad]
            rot_rect2: Second rotated rectangle ((cx, cy), (w, h), angle_deg) or [cx, cy, w, h, angle_rad]
            
        Returns:
            float: Angle difference in degrees between the top edges (0-90 degrees)
        """
        try:
            # Handle different input formats
            if isinstance(rot_rect1, (list, tuple)) and len(rot_rect1) == 5:
                # Format: [cx, cy, w, h, angle_rad]
                cx1, cy1, w1, h1, angle1_rad = rot_rect1
                angle1_deg = math.degrees(angle1_rad)
                rect1 = ((cx1, cy1), (w1, h1), angle1_deg)
            else:
                # Format: ((cx, cy), (w, h), angle_deg)
                rect1 = rot_rect1
                angle1_deg = rect1[2]
            
            if isinstance(rot_rect2, (list, tuple)) and len(rot_rect2) == 5:
                # Format: [cx, cy, w, h, angle_rad]
                cx2, cy2, w2, h2, angle2_rad = rot_rect2
                angle2_deg = math.degrees(angle2_rad)
                rect2 = ((cx2, cy2), (w2, h2), angle2_deg)
            else:
                # Format: ((cx, cy), (w, h), angle_deg)
                rect2 = rot_rect2
                angle2_deg = rect2[2]
            
            # Get box points for both rectangles
            box1_points = cv2.boxPoints(rect1)
            box2_points = cv2.boxPoints(rect2)
            
            # Find top edges (edges with smallest y values)
            def get_top_edge_direction(box_points):
                # Find the edge with the smallest average y coordinate
                edges = []
                for i in range(4):
                    p1 = box_points[i]
                    p2 = box_points[(i + 1) % 4]
                    ...
                    edges.append((avg_y, edge_direction))
                
                # Return the direction of the edge with smallest y
                edges.sort(key=lambda x: x[0])
                return edges[0][1]  # Return direction vector
            
            top_edge1_dir = get_top_edge_direction(box1_points)
            top_edge2_dir = get_top_edge_direction(box2_points)
            
            # Calculate angle between the two direction vectors
            dot_product = np.clip(np.dot(top_edge1_dir, top_edge2_dir), -1.0, 1.0)
            angle_rad = math.acos(abs(dot_product))  # Use abs to get acute angle
            angle_deg = math.degrees(angle_rad)
            
            # Ensure the result is in [0, 90] range
            if angle_deg > 90:
            ...
            print(f"Error calculating rectangles top edge angle: {e}")
            return 0.0
    
    @staticmethod
    def calculate_rotated_rect_bbox_intersection_mask(affordance_rot_rect, bbox, shape):
        """
        Calculate intersection mask between rotated rectangle and normal bbox.
        
        Args:
            affordance_rot_rect: [xc, yc, w, h, angle_rad] rotated rectangle
            bbox: [x1, y1, x2, y2] normal rectangle
            shape: (height, width) for creating masks
            
        Returns:
            np.ndarray: Binary mask of intersection area
        """
        try:
            h, w = shape
            ...
            xc, yc, width, height, angle_rad = affordance_rot_rect
            rect = ((float(xc), float(yc)), (float(width), float(height)), float(angle_rad) * 180 / math.pi)
            box_points = cv2.boxPoints(rect).astype(int)
            cv2.fillPoly(affordance_mask, [box_points], 255)
            ...
            intersection_mask = np.logical_and(affordance_mask > 0, bbox_mask > 0).astype(np.uint8) * 255
            
            return intersection_mask
            
        except Exception as e:
            print(f"Error calculating rotated rect bbox intersection mask: {e}")
            h, w = shape
            return np.zeros((h, w), dtype=np.uint8)
    
    @staticmethod
    def calculate_mask_intersection(mask1, mask2):
        """
        Calculate intersection between two binary masks.
        
        Args:
            mask1: First binary mask (values 0 or 255)
            mask2: Second binary mask (values 0 or 255)
            
        Returns:
            np.ndarray: Binary mask of intersection area
        """
        try:
            # Normalize masks to binary (0 or 1)
            binary_mask1 = (mask1 > 0).astype(np.uint8)
            binary_mask2 = (mask2 > 0).astype(np.uint8)
            
            # Calculate intersection
            intersection_mask = np.logical_and(binary_mask1, binary_mask2).astype(np.uint8) * 255
            
            return intersection_mask
            
        except Exception as e:
            print(f"Error calculating mask intersection: {e}")
            return np.zeros_like(mask1, dtype=np.uint8)
    
    @staticmethod
    def calculate_mask_overlap_ratio(overlap1_mask, overlap2_mask):
        """
        Calculate overlap ratio: area(overlap2) / area(overlap1).
        
        Args:
            overlap1_mask: First overlap mask
            overlap2_mask: Second overlap mask
            
        Returns:
            float: Overlap ratio (0.0 to 1.0)
        """
        try:
            area1 = np.sum(overlap1_mask > 0)
            area2 = np.sum(overlap2_mask > 0)
            ...
            print(f"Error calculating mask overlap ratio: {e}")
            return 0.0
