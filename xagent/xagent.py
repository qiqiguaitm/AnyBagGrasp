...
4. 确保动作序列能完成任务要求"""
    ...
    def describe_objects(self, image_path: str, model: Optional[str] = None) -> Dict[str, str]:
        ...
        analysis_prompt = f"""请分析图像中的物体特征，并与检测结果对应。

系统检测到的物体（你必须从以下坐标中选择，不能自创坐标）：
{object_info_text}

**物体分析：**
请为图像中看到的每个物体填写（位置和尺寸必须从上述检测结果中选择）：

- 物体ID：[颜色+类型，如"粉色手提袋"]
  - 类别：手提袋
  - 颜色：[观察到的颜色]
  - 大小：[从上述对象中选择对应的尺寸，如234x405]
  - 位置：[从上述对象中选择对应的坐标，如(981,468)]
  - 材质：[如果可见]

示例（必须使用检测到的实际坐标）：
- 如果粉色袋子在画面中间偏右，应该对应对象2的位置(710,469)
- 如果黄色袋子在画面最右侧，应该对应对象3的位置(1258,460)
- 如果蓝色袋子在画面最左侧，应该对应对象4的位置(159,350)
